...
const Dashboard: React.FC<DashboardProps> = ({ requests }) => {
  ...
  const stats = [
    { label: 'Solicitações Totais', value: requests.length, icon: 'fa-list-check', color: 'bg-navy', detail: 'Volume geral' },
    { label: 'Em Tratamento', value: requests.filter(r => r.estado === RequestStatus.IN_PROGRESS).length, icon: 'fa-sync-alt', color: 'bg-moss', detail: 'Ações em curso' },
    { label: 'Tempo Médio', value: `${avgDays}d`, icon: 'fa-hourglass-half', color: 'bg-amber-600', detail: 'Média de dias' },
    { label: 'Alertas Críticos', value: alertCount, icon: 'fa-triangle-exclamation', color: 'bg-red-600', detail: `Acima de ${avgDays}d` },
    { label: 'Finalizado', value: requests.filter(r => r.estado === RequestStatus.FINISHED).length, icon: 'fa-check-double', color: 'bg-green-600', detail: 'Concluídos' },
    { label: 'Consulta', value: requests.filter(r => r.tratamento.toLowerCase().includes('consulta')).length, icon: 'fa-search', color: 'bg-blue-500', detail: 'Arquivos em consulta' },
    { label: 'Indexação', value: requests.filter(r => r.tratamento.toLowerCase().includes('indexação') || r.tratamento.toLowerCase().includes('indexacao')).length, icon: 'fa-folder-tree', color: 'bg-purple-600', detail: 'Catalogação' },
    { label: 'Digitalização', value: requests.filter(r => r.tratamento.toLowerCase().includes('digitalização') || r.tratamento.toLowerCase().includes('digitalizacao')).length, icon: 'fa-file-pdf', color: 'bg-cyan-600', detail: 'Documentos capturados' }
  ];
